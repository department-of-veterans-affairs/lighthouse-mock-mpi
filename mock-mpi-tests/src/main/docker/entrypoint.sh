...
# Make 1305 search by attributes request.
thirteenOhFiveSearchByAttributes() {
  local otzf=$(mktemp)
...
# Make 1305 search by icn request.
thirteenOhFiveSearchByIcn() {
  local otzf=$(mktemp)
...
httpListenerTests () {
  ...
  for body in "$(thirteenOhFiveSearchByAttributes)" "$(thirteenOhFiveSearchByIcn)" "$(thirteenOhNine)"
  do
